...
import com.bbt.ghostroom.adapters.NewGroupAdapter
import com.bbt.ghostroom.adapters.NewGroupSelectedUsersAdapter
...
import com.bbt.ghostroom.utils.network.BroadcastManager

import com.bbt.ghostroom.utils.network.GroupManager
import com.bbt.ghostroom.views.dialogs.SetGroupTitleDialog
...
class NewGroupActivity : ForwardActivity(), ForwardAdapter.OnUserClick,
    NewGroupSelectedUsersAdapter.OnUserClick, ForwardActivity.SearchCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fabNext.setOnClickListener {
            if (intent.hasExtra(IntentUtils.UID)) {
                val data = Intent()
                data.putExtra(
                    IntentUtils.EXTRA_SELECTED_USERS,
                    selectedUsers as ArrayList<out Parcelable?>?
                )
                ...
                dialog.setmListener { groupTitle ->
                    if (isBroadcast) createBroadcast(groupTitle) else createGroup(
                        groupTitle
                    )
                }
                dialog.show()
        ...
        rvGroup.setInsetPadding()
        fabNext.setInsetPadding(
            plusLeft = fabNext.marginLeft,
            plusBottom = fabNext.bottom,
            plusRight = fabNext.marginRight
        )
    }
    ...
    private fun createBroadcast(broadcastName: String) {
        ...
        if (NetworkHelper.isConnected(this)) {
            broadcastManager.createNewBroadcast(broadcastName, selectedUsers.toList())
                .subscribe { broadcastUser, throwable ->
                    progressDialog.dismiss()

                    if (throwable != null) {

                        Toast.makeText(this@NewGroupActivity, R.string.error, Toast.LENGTH_SHORT)
                            .show()
    ...
    private fun createGroup(groupTitle: String) {
        ...
        progressDialog.show()
        if (NetworkHelper.isConnected(this)) {
            groupManager.createNewGroup(groupTitle, selectedUsers)
                .subscribe { groupUser, throwable ->
                    progressDialog.dismiss()
    ...
    private fun setAdapter() {
        ...
        rvSelectedUsersNewGroup.layoutManager =
    ...
    override fun onChange(user: User, added: Boolean) {
        val count = selectedForwardedUsers?.size ?: 0
        val position = selectedUsers.indexOf(user)
        if (added) {
            if (count + EXTRA_COUNT > maxNumberUsers) {
                Toast.makeText(this, R.string.max_number_of_users_reached, Toast.LENGTH_SHORT)
                    .show()
            } else {
    ...
    private val maxNumberUsers: Int
        private get() = if (isBroadcast) resources.getInteger(R.integer.max_broadcast_users_count) else resources.getInteger(
            R.integer.max_group_users_count
        )

    //this will called when a user clicks on a user in horizontal list
    override fun onRemove(user: User) {
        //remove selected circle
        val viewHolderForAdapterPosition = rvGroup.findViewHolderForAdapterPosition(
            users?.indexOf(user)
                ?: 0
        )
        val selectedCircle =
            viewHolderForAdapterPosition?.itemView?.findViewById<HidelyImageView>(R.id.img_selected)
        selectedCircle?.hide()
