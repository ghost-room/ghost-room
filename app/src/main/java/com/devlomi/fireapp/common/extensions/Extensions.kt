package com.bbt.ghostroom.common.extensions
...
fun FragmentManager.findFragmentByTagForViewPager(
...
fun AudioManager.requestAudioFocusCompat(
    onFocusChangeListener: AudioManager.OnAudioFocusChangeListener,
    streamType: Int,
    gain: Int
): AudioFocusRequest? {
...
fun AudioManager.abandonAudioFocusCompat(
    onFocusChangeListener: AudioManager.OnAudioFocusChangeListener?,
...
fun View.setInsetPadding(useTop: Boolean = false) {
    ViewCompat.setOnApplyWindowInsetsListener(this) { v, windowInsets ->
        val insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars())
        v.setPadding(insets.left, if (useTop) insets.top else 0, insets.right, insets.bottom)
        WindowInsetsCompat.CONSUMED
    }
}

fun View.getInsets(callback: (insets: Insets) -> Unit) {
    ViewCompat.setOnApplyWindowInsetsListener(this) { v, windowInsets ->
        val insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars())
        callback(insets)
        WindowInsetsCompat.CONSUMED
...
fun View.setInsetPadding(
    ...
    ViewCompat.setOnApplyWindowInsetsListener(this) { v, windowInsets ->
        val insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars())
        v.setPadding(
            insets.left + plusLeft,
            if (useTop) insets.top else 0,
            insets.right + plusRight,
            insets.bottom + plusBottom
        )
        WindowInsetsCompat.CONSUMED
    }
}

fun View.setInsetMargin(useTop: Boolean = false) {
    ViewCompat.setOnApplyWindowInsetsListener(this) { v, windowInsets ->
        val insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars())
        val lp = v.layoutParams
        if (lp is ViewGroup.MarginLayoutParams) {
            lp.leftMargin = insets.left
            lp.topMargin = if (useTop) insets.top else 0
...
fun View.setInsetMargin(
    ...
    ViewCompat.setOnApplyWindowInsetsListener(this) { v, windowInsets ->
        ...
        val lp = v.layoutParams
        if (lp is ViewGroup.MarginLayoutParams) {
            lp.leftMargin = insets.left + plusLeft
            lp.topMargin = if (useTop) insets.top else 0
            lp.rightMargin = insets.right + plusRight
            lp.bottomMargin = insets.bottom + plusBottom
            v.layoutParams = lp
        }
        WindowInsetsCompat.CONSUMED
    }
...
fun OnBackPressedCallback.superOnBackPressed(onBackPressedDispatcher: OnBackPressedDispatcher) {
    isEnabled = false
