...
import com.bbt.ghostroom.model.realms.RealmLocation;
import com.bbt.ghostroom.model.realms.User;
import com.bbt.ghostroom.placespicker.Place;
import com.bbt.ghostroom.utils.enc.EncryptionTypeUseCase;
...
public class MessageCreator {
    ...
    private static Message createImageMessage(User user, final String imagePath, boolean fromCamera) {
        ...
        // then we need to delete the captured image after copying it to another
        // directory
        if (fromCamera) {
    ...
    private static Message createVideoMessage(Context context, User user, final String path) {
        // REMINDER we do not copy the original file if the user chose a video from
        // gallery because it may has a Big Size
        final Message message = new Message();
        final String pushKey = FireConstants.messages.push().getKey();
    ...
    // create multiple contact messages since the user may select multiple contacts
    // to send
    ...
    public static Message createForwardedMessage(Message mMessage, User user, String fromId) {
        ...
        if (user.isBroadcastBool()) {
            // since the user param comes from parcelize, it doesn't contain full info, like
            // broadcast
            ...
            try {
                ...
                switch (type) {
                    ...
                    case MessageType.SENT_VIDEO:
                        ...
                        File videoFile = BuildVerUtil.isApi29OrAbove() && !fromCamera ? DirManager.generateFile(type)
                                : new File(pathOrUri);
                    ...
                    case MessageType.SENT_AUDIO:
                        if (BuildVerUtil.isApi29OrAbove()) {
                            ...
                            String audioFileExtension = MediaStoreFileInfo.INSTANCE
                                    .getFileExtensionUsingMimeType(MyApp.context(), uri);
                    ...
                    case MessageType.SENT_LOCATION:
                        ...
                        RealmLocation location = new RealmLocation(latLng.latitude, latLng.longitude, addressName,
                                placeName);
                        message.setLocation(location);
                        break;
