...
public class DirManager {
    ...
    // this will generate a new file name it will generate the date as year month
    // day Millisecond
    // more info
    ...
    public static String generateNewName(int type) {
        Date date = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddSSSS", Locale.US); // the Locale us is to use english
                                                                                // numbers
        return getFileTypeString(type) + "-" + sdf.format(date);
    }

    // create the file to save the user profile into it
    // :/sdcard/FireApp/ProfilePhotos/IMG-"DATE".jpg
    ...
    // this used because we need the extension of the file so we can save the file
    // with its extension
    ...
    // here we don't want to generate a new name,instead the name will be formatted
    // from sender
    ...
    // this will prevent the gallery or audio player app in the device from showing
    // "sent" files (images,videos,audio,voice messages) by our app
    ...
    public static File getReceivedStatusFile(String statusId, int type) {
        // if the status was downloaded from other users we will save it in the hidden
        // folder ".Statuses"
