...
import com.bbt.ghostroom.exceptions.BackupFileMismatchedException
...
class RealmBackupRestore {
    ...
    fun backup(): File {
        ...
        val tmpRealmFile = File(context().cacheDir, EXPORT_REALM_FILE_NAME)

        // if backup file already exists, delete it
        exportRealmFile.delete()
        tmpRealmFile.delete()
        // copy current realm to backup file
        //Realm doesn't support Scoped Storage yet, therefore we have to copy the realm file from the app's cache directory
        realm.writeCopyTo(tmpRealmFile)
        tmpRealmFile.copyTo(exportRealmFile)
        tmpRealmFile.delete()
        realm.close()
