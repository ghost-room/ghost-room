...
import com.bbt.ghostroom.model.constants.NetworkType;
...
public class SharedPreferencesManager {
    ...
    // check what user is enabled (video,audio,images) for every state
    ...
    synchronized public static void init(Context context) {
        ...
        if (defaultWifiSet == null)
            defaultWifiSet = new HashSet<>(
                    Arrays.asList(context.getResources().getStringArray(R.array.autodownload_wifi_defaults)));

        if (defaultCellularSet == null)
            defaultCellularSet = new HashSet<>(
                    Arrays.asList(context.getResources().getStringArray(R.array.autodownload_cellular_defaults)));

        if (defaultRoamingSet == null)
            defaultRoamingSet = new HashSet<>(
                    Arrays.asList(context.getResources().getStringArray(R.array.autodownload_roaming_defaults)));
    ...
    // this will return if user has enabled auto download for certain network type
    // and for certain media type
    public static boolean canDownload(int mediaType, int availableNetworkType) {
        ...
        if (mediaType == MessageType.RECEIVED_VOICE_MESSAGE
                || mediaType == MessageType.RECEIVED_STICKER)
            return true;
    ...
    public static Uri getRingtone() {
        return Uri.parse(mSharedPref.getString("notifications_new_message_ringtone",
