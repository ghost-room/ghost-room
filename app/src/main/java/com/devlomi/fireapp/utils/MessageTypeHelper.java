...
public class MessageTypeHelper {
    ...
    public static String getMessageContent(Message message, boolean includeEmoji) {
        ...
        if (message.isTextMessage())
            ...
            // if it's a voice message add mic icon at the start along with voice message
            // duration in ()
            if (message.isVoiceMessage()) {
                ...
                contentText = emojiText + message.getContact().getName()
                        + MessageTypeHelper.getTypeText(message.getType());
            } else
