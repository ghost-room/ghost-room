...
import com.bbt.ghostroom.model.constants.DBConstants;
...
import com.bbt.ghostroom.model.realms.Broadcast;
import com.bbt.ghostroom.model.realms.Chat;
import com.bbt.ghostroom.model.realms.CurrentUserInfo;
import com.bbt.ghostroom.model.realms.DeletedMessage;
import com.bbt.ghostroom.model.realms.FireCall;
import com.bbt.ghostroom.model.realms.Group;
import com.bbt.ghostroom.model.realms.JobId;
import com.bbt.ghostroom.model.realms.Message;
import com.bbt.ghostroom.model.realms.PendingGroupJob;
import com.bbt.ghostroom.model.realms.Status;
import com.bbt.ghostroom.model.realms.StatusSeenBy;
import com.bbt.ghostroom.model.realms.TempMessage;
import com.bbt.ghostroom.model.realms.UnUpdatedStat;
import com.bbt.ghostroom.model.realms.UnUpdatedVoiceMessageStat;
import com.bbt.ghostroom.model.realms.User;
import com.bbt.ghostroom.model.realms.UserStatuses;
...
public class RealmHelper {
    ...
    public void deleteTempMessage(String messageId) {
        RealmResults<TempMessage> tempMessages = realm.where(TempMessage.class)
                .equalTo(DBConstants.MESSAGE_ID, messageId).findAll();
    ...
    private void updateLastMessageForChat(String chatId, Message messageToDelete, Chat chat) {
        if (chat != null) {
            ...
            if (lastMessage != null && lastMessage.getMessageId().equals(messageToDelete.getMessageId())) {
                RealmResults<Message> messagesInChat = realm.where(Message.class).equalTo(DBConstants.CHAT_ID, chatId)
                        .findAll();
                int messagesCount = messagesInChat.size();
                // check if there is more than one message in this chat
                if (messagesCount > 1) {
                    // get the message before the last message (the new message to set it as the
                    // last message)
    ...
    public Message getMessage(String messageId, String chatId) {
        ...
        return realm.where(Message.class).equalTo(DBConstants.MESSAGE_ID, messageId)
                .equalTo(DBConstants.CHAT_ID, chatId).findFirst();
    }
    ...
    // therefore we want to get chatId to get the message that has broadcast
    // messageId
    ...
    // get received messages that are not read to update them in Firebase database
    // as read
    ...
    // get unread sent voice messages so we can add listeners for them once they
    // have changed
    ...
    // update upload state when it's finished whether it's success ,failed,loading
    // or cancelled
    public void updateDownloadUploadStat(String messageId, int downloadUploadStat) {
        // we are getting all messages because it's may be a broadcast ,if so we want to
        // update the state of all of them
    ...
    // update download state when it's finished whether it's success ,failed,loading
    // or cancelled
    ...
    // save the firebase storage path for a file in realm to use it late when
    // forwarding
    ...
    // this is called when user has no internet connection and he opened a chat to
    // see a message
    // that is NOT read before,therefore we want to save this to update the message
    // with read state
    ...
    public void saveUnUpdatedVoiceMessageStat(String myUid, String messageId, boolean isVoiceMessageSeen) {
        UnUpdatedVoiceMessageStat unUpdatedVoiceMessageStat = new UnUpdatedVoiceMessageStat(myUid, messageId,
                isVoiceMessageSeen);
        realm.beginTransaction();
    ...
    public void deleteUnUpdatedVoiceMessageStat(String messageId) {
        realm.where(UnUpdatedVoiceMessageStat.class).equalTo(DBConstants.MESSAGE_ID, messageId).findAll()
                .deleteAllFromRealm();
    }
    ...
    public void deletePendingGroupCreationJob(String groupId) {
        RealmResults<PendingGroupJob> pendingGroupJobs = realm.where(PendingGroupJob.class).equalTo("groupId", groupId)
                .findAll();
    ...
    public int getJobId(String messageId, boolean isVoiceMessage) {
        JobId jobId = realm.where(JobId.class).equalTo(DBConstants.ID, messageId)
    ...
    public String getJobId(int id, boolean isVoiceMessage) {
        JobId jobId = realm.where(JobId.class).equalTo(DBConstants.JOB_ID, id)
                .equalTo(DBConstants.isVoiceMessage, isVoiceMessage).findFirst();
        if (jobId != null) {
            return jobId.getId();
        }
    ...
    public void deleteJobId(String id, boolean isVoiceMessage) {
        JobId jobId = realm.where(JobId.class).equalTo(DBConstants.ID, id)
                .equalTo(DBConstants.isVoiceMessage, isVoiceMessage).findFirst();
        if (jobId != null) {
            realm.beginTransaction();
    ...
    public RealmResults<UserStatuses> getAllStatuses() {
        return realm.where(UserStatuses.class)
                ...
                .isNotEmpty("statuses")
                .findAll()
                .sort(DBConstants.ARE_ALL_STATUSES_SEEN, Sort.ASCENDING, DBConstants.lastStatusTimestamp,
                        Sort.DESCENDING);
    }
    ...
    public UserStatuses getUserStatuses(String userId) {
        return realm.where(UserStatuses.class).equalTo(DBConstants.statusUserId, userId).isNotEmpty("statuses")
                .findFirst();
    }
    ...
    // local database does not has certain id in the new ids then it's probably
    // deleted
