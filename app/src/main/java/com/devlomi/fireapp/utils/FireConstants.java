...
public class FireConstants {
    ...
    // this is used when an admin removes a group member so the removed user will
    // not be able
    ...
    private static DatabaseReference deleteMessageRequestsForBroadcast = mainRef
            .child("deleteMessageRequestsForBroadcast");
    ...
    // this ref is for the user state is he online or not ,if he is not online this
    // will contain the last seen timestamp
    public static final DatabaseReference presenceRef = mainRef.child("presence");

    // this will have the user typing or recording or do nothing value when he
    ...
    // this will get the user id by his phone number to use it when searching for a
    // user
    ...
    // MAX SIZE FOR FCM message IS 4096 ,however we want some more space for other
    // items regardless "Content"
    ...
    public static DatabaseReference getDeleteMessageRequestsRef(String messageId, boolean isGroup, boolean isBroadcast,
            String groupOrBroadcastId) {
        if (isGroup)
