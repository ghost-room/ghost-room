...
import com.bbt.ghostroom.job.NetworkJobService;
import com.bbt.ghostroom.job.SaveTokenJob;
import com.bbt.ghostroom.model.constants.MessageType;
import com.bbt.ghostroom.model.realms.GroupEvent;
...
public class ServiceHelper {
    ...
    // this will fire a Network request (download or upload request or event a
    ...
    public static void startNetworkRequest(Context context, String messageId, String chatId) {
        ...
        if (BuildVerUtil.isOreoOrAbove()) {
            PersistableBundle bundle = new PBundleUtil.Builder(messageId).action(action).messageId(messageId)
                    .chatId(chatId).build().getBundle();
    ...
    // this will update the received message stat and set it as Received or read in
    // Firebase database
    public static void startUpdateMessageStatRequest(Context context, String messageId, String myUid, String chatId,
            int statToBeUpdated) {
        if (BuildVerUtil.isOreoOrAbove()) {
            PersistableBundle bundle = new PBundleUtil.Builder(messageId)
                    .action(IntentUtils.INTENT_ACTION_UPDATE_MESSAGE_STATE)
                    .messageId(messageId)
                    .myUid(myUid)
                    .chatId(chatId)
    ...
    // this will update the received voice message stat and set it as seen in
    // Firebase database
    public static void startUpdateVoiceMessageStatRequest(Context context, String messageId, String chatId,
            String myUid) {
        if (BuildVerUtil.isOreoOrAbove()) {
            PersistableBundle bundle = new PBundleUtil.Builder(messageId)
    ...
    public static void updateGroupInfo(Context context, String groupId, GroupEvent groupEvent) {
        if (BuildVerUtil.isOreoOrAbove()) {
            ...
            PersistableBundle bundle = new PBundleUtil.Builder(groupId).action(IntentUtils.INTENT_ACTION_UPDATE_GROUP)
                    .groupId(groupId).groupEvent(groupEvent).build().getBundle();
            NetworkJobService.schedule(context, groupId, bundle);
    ...
    public static void headsetStateChanged(Context context, int currentHeadsetState) {
        Intent intent = new Intent(context, AudioService.class).putExtra(IntentUtils.EXTRA_HEADSETSTATE_CHANGED,
                currentHeadsetState);
        context.startService(intent);
